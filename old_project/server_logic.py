...
class Peer:
    ...
    def send(self, vectors: ListOfVectorsComplete, status: str, id, retries=3):
        logger.debug(
            f"--> [Peer Send] Sending {len(vectors)} vectors to Peer {self.id} ({self.grpc_url}) | Status: {status}"
        )
        ...
        req = p2p_pb2.AddVectorsRequest(req_id=id, content=proto_vectors, type=status)
        ...
        for attempt in range(retries):
            try:
                ...
                logger.warning(
                    f"--> [Peer Send] Attempt {attempt + 1}/{retries} FAILED to {self.id}: {e}"
                )
                time.sleep(0.5 * (attempt + 1))  # Exponential backoff

        logger.error(
            f"--> [Peer Send] PERMANENT FAILURE to {self.id} after {retries} attempts."
        )
        return False

    def send_clusters(self, assignment, clusters, meta_hnsw, request_id, retries=3):
        logger.debug(
            f"--> [Peer SendClusters] Sending cluster assignment to Peer {self.id}"
        )
        to_send_dict = {
            "my_vectors": [
                vector_complete
        ...
        for attempt in range(retries):
            try:
                ...
                logger.debug(
                    f"--> [Peer SendClusters] Success: Sent clusters to {self.id}"
                )
                return True
            except grpc.RpcError as e:
                logger.warning(
                    f"--> [Peer SendClusters] Attempt {attempt + 1}/{retries} FAILED to {self.id}: {e}"
                )
                time.sleep(1)
    ...
    def query_peer(self, query: ListOfVectors, topk: int, request_id: int, retries=3):
        logger.debug(
            f"--> [Peer Query] Querying Peer {self.id} for {len(query)} vectors"
        )
        # query contains tuples (vector, cluster_id)
        proto_query = [
            p2p_pb2.VectorList(values=v[0], cluster_id=str(v[1])) for v in query
        ]
        req = p2p_pb2.QueryRequest(
            req_id=request_id, query_vectors=proto_query, topk=topk
        )
        ...
        for attempt in range(retries):
            try:
                ...
                logger.debug(
                    f"--> [Peer Query] Success: Peer {self.id} returned {len(results)} results"
                )
                return results
            except grpc.RpcError as e:
                logger.warning(
                    f"--> [Peer Query] Attempt {attempt + 1}/{retries} FAILED to {self.id}: {e}"
                )
                time.sleep(0.5)
...
class ServerApp:
    def __init__(
        ...
        num_vectors_before_clustering=10_000,
        dimension=384,
        batch_size=256,
        batch_size_retry=64,
    ):
        ...
        self.peer_executor = ThreadPoolExecutor(
            max_workers=10, thread_name_prefix="PeerComm"
        )

        # Start worker thread
        self.worker_thread = threading.Thread(
            target=self._process_queue, name="QueueWorker", daemon=True
        )
        self.worker_thread.start()

        logger.info(
            f"ServerApp initialized: ID={id}, URL={url}, Coords={coordinator_url}"
        )

    def create_cluster_collections(self):
        logger.warning(
            f"Creating collections for {len(self.node_clusters)} clusters: {[c[0] for c in self.node_clusters]}"
        )
    ...
    def _process_queue(self):
        ...
        while True:
            try:
                ...
                if self.status == ServerStatus.BOOTSTRAP:
                    if self.i_am_coord():
                        ...
                        if buffer_len % 1000 == 0:
                            logger.info(
                                f"[Worker] Bootstrap Buffer: {buffer_len}/{self.num_vectors_before_clustering}"
                            )
                        ...
                        if sender_status == "clustered":
                            ...
                            logger.info(
                                "[Worker] Received clustered vectors while in BOOTSTRAP. Waiting for SetClusters..."
                            )
                            start_wait = time.time()
                            while self.status == ServerStatus.BOOTSTRAP:
                                if time.time() - start_wait > 30:  # 30s timeout
                                    logger.error(
                                        "[Worker] Timeout waiting for SetClusters. Dropping vectors."
                                    )
                                    break
                                time.sleep(0.1)
                            ...
                            if self.status == ServerStatus.CLUSTERED:
                                logger.info(
                                    "[Worker] Status changed to CLUSTERED. Processing vectors."
                                )
                                qdrant_module.insert_vectors_generic(
                                    self.qdrant_url,
                                    self.collection_name,
                                    vectors,
                                    self.batch_size_retry,
                                    self.batch_size,
                                )
                        else:
                            logger.debug(
                                f"[Worker] Received {len(vectors)} vectors in BOOTSTRAP (Not Coord). Dropping or waiting?"
                            )
                ...
                logger.error(
                    f"[Worker] Error processing queue item: {e}", exc_info=True
                )
    ...
    def _perform_clustering(self, request_id):
        with metrics.REQUEST_LATENCY.labels(operation="clustering").time():
            ...
            assignment = clustering_module.get_node_assignment(
                clusters, peers_with_me, self.replicas
            )
            self.meta_hnsw = clustering_module.build_meta_hnsw(clusters, self.dimension)

            [
                logger.info(
                    f"Clusters/Vectors per node: {nid}: {len(c_list)} clusters, {sum([len(clusters[cid][1]) for cid, _ in c_list])} vectors"
                )
                for nid, c_list in assignment.items()
            ]
            ...
            def broadcast_to_peer(peer):
                return (
                    peer.id,
                    peer.send_clusters(
                        assignment, clusters, self.meta_hnsw, request_id
                    ),
                )
            ...
            if failed_broadcasts:
                logger.error(
                    f"[Clustering] Failed to broadcast to: {failed_broadcasts}"
                )
            else:
            ...
            local_vectors = [
                (v, v_id, v_payload, str(cluster_id))
                for cluster_id, _ in assignment[self.node_id]
                for v, v_id, v_payload, _ in clusters[cluster_id][1]
            ]
            logger.info(
                f"Clustering: Inserting {len(local_vectors)} assigned vectors locally."
            )
    ...
    def add_vectors(
        self, vectors: ListOfVectorsComplete, status_of_sender: str, request_id
    ):
        """
        ...
        with metrics.REQUEST_LATENCY.labels(operation="add_vectors").time():
            ...
            logger.debug(
                f"[AddVectors] Enqueued {len(vectors)} vectors. Queue size: {self.vector_queue.qsize()}"
            )
    ...
    def add_vectors_client(self, vectors: ListOfVectorsWithPayload, request_id):
        with metrics.REQUEST_LATENCY.labels(operation="add_vectors_client").time():
            logger.debug(
                f"[Client Request] Adding {len(vectors)} vectors. Status: {self.status.value}"
            )
            vectors_with_id = [
                (vector_content, self.get_id(), vector_payload, "-1")
                for vector_content, vector_payload in vectors
            ]
    ...
    def set_clusters(self, assignment, request_id):
        with metrics.REQUEST_LATENCY.labels(operation="set_clusters").time():
            ...
            self.meta_hnsw = clustering_module.MetaHNSW.from_serializable_dict(
                assignment["meta_hnsw"]
            )
            self.create_cluster_collections()
            ...
            logger.info(
                f"[Set Clusters] Storing {len(my_vecs)} assigned vectors locally."
            )
    ...
    def route_vectors_send(self, vectors: ListOfVectorsComplete, request_id):
        with metrics.REQUEST_LATENCY.labels(operation="route_vectors_send").time():
            ...
            target_clusters_batch = clustering_module.find_batch(
                self.meta_hnsw, raw_vectors, 1
            )
            ...
            for i, (
                ...
                if found < self.replicas:
                    logger.warning(
                        f"[ROUTER] not enough replicas found: {found} (Cluster ID: {top_1})"
                    )
            ...
            def send_to_peer(peer, vectors_chunk):
                ...
                if len(vectors_chunk) > CHUNK_SIZE:
                    for i in range(0, len(vectors_chunk), CHUNK_SIZE):
                        peer.send(
                            vectors_chunk[i : i + CHUNK_SIZE], "clustered", request_id
                        )
                else:
    ...
    def linear_search(self, query: ListOfVectors, topk: int):
        with metrics.REQUEST_LATENCY.labels(operation="linear_search").time():
            ...
            logger.debug(
                f"[Linear Search] Searching buffer ({len(total_vectors)} vectors)."
            )
            ...
            for qv in query:
                ...
                distances_calcs = [
                    (vector, utils.cosine_similarity(vector[0], query_vec))
                    ...
                    [
                        {
                            "id": v[1],
                            "score": d,
                            "payload": {"string": v[2], "vector": v[0]},
                        }
                        for v, d in distances_calcs[:topk]
                    ]
                )
            return to_return
    ...
    def query_me(self, query, topk: int):
        with metrics.REQUEST_LATENCY.labels(operation="local_query").time():
            if (
                self.status in [ServerStatus.BOOTSTRAP, ServerStatus.CLUSTERING]
                and self.i_am_coord()
            ):
                logger.debug(
                    "[Local Query] Bootstrap/Clustering mode: performing linear search on buffer."
                )
                return self.linear_search(query, topk)

            results = qdrant_module.query_vectors_generic(
                self.qdrant_url, self.collection_name, query, topk
            )
            return results
    ...
    def query(self, query: ListOfVectors, topk: int, request_id: int):
        with metrics.REQUEST_LATENCY.labels(operation="global_query").time():
            logger.info(
                f"[Global Query] Processing query (ReqID: {request_id}). Queries: {len(query)}, TopK: {topk}"
            )
            response = []
            ...
            for index, peer in enumerate(self.peers):
                if to_query_peer[index]:
                    ...
                    metrics.VECTORS_ROUTED.labels(target_node=peer.id).inc(
                        len(to_query_peer[index])
                    )
            ...
            # Log Routing
            q_summary = [
                f"Peer {peer.id}: {len(to_query_peer[i])}"
            ...
            def query_peer_task(peer, queries):
                try:
                    with metrics.PEER_LATENCY.labels(
                        peer_id=peer.id, operation="query"
                    ).time():
            ...
            if to_query_me:
                tasks.append(
                    (lambda: (self.node_id, True, self.query_me(to_query_me, topk)), ())
                )
            ...
            unique_response = sorted(
                unique_response, key=lambda x: x["score"], reverse=True
            )
            ...
            if self.status == ServerStatus.CLUSTERED:
                ...
                for cluster in self.node_clusters:
                    ...
                    collection_name = utils.get_collection_name(
                        self.collection_name, cluster_id
                    )
    ...
    def metrics_snapshot(self):
        return {"query_received_count": self.query_received_count}
