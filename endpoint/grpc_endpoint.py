...
class P2PNodeServicer(p2p_pb2_grpc.P2PNodeServicer):
    ...
    async def GetId(self, request, context):
        try:
            ...
            # print(f"Error GetId: {e}")
            return p2p_pb2.IdResponse()
    ...
    async def Similarity(self, request, context):
        try:
            ...
            return p2p_pb2.SimilarityResponse()
    ...
    async def SearchVectorsLocal(self, request, context):
        try:
            ...
            results = await self._run(self.server.search_vectors_local, q_vecs, request.top_k)
            
            resp_items = []
            for r in results:
                resp_items.append(p2p_pb2.SearchResultItem(
                    vector=r[0], id=r[1], payload=r[2], similarity=r[3]
                ))
            return p2p_pb2.SearchResponse(results=resp_items)
        except Exception as e:
            # print(f"Error SearchVectorsLocal: {e}")
    ...
    async def GetVectorDigest(self, request, context):
        try:
            ...
            for vid, ver in digest.items():
                proto_digest[vid] = p2p_pb2.VersionInfo(timestamp=ver[0], node_id=ver[1])
            
            return p2p_pb2.DigestResponse(digest=proto_digest)
        except Exception as e:
            # print(f"Error GetVectorDigest: {e}")
            return p2p_pb2.DigestResponse()
        ...
        try:
            ...
            for v in vecs:
                 ...
                 vc = p2p_pb2.VectorComplete(
                     values=list(v[0]), id=v[1], payload=v[2], cluster_id=v[3],
                     timestamp=ver[0], version_node_id=ver[1], destinations=dests
                 )
                 proto_vecs.append(vc)
                 
            return p2p_pb2.VectorList(vectors=proto_vecs)
        except Exception as e:
            # print(f"Error GetVectorsByIds: {e}")
            return p2p_pb2.VectorList()
    ...
    async def GetPartitionCoordinatorId(self, request, context):
        try:
             ...
             return p2p_pb2.IdResponse(id=-1)
...
class GRPCEndpoint(BaseEndpoint):
    """
    Endpoint implementation using gRPC protocol.
    """
    ...
    async def _run(self, method, *args):
        # Helper to run sync server methods in executor
        return await self.loop.run_in_executor(None, lambda: method(*args))
    ...
    async def Receive(self, request, context):
        try:
            # Convert proto vectors to python tuples
            vectors = []
            for v in request.vectors:
    ...
    async def SetClusters(self, request, context):
        try:
            # Reconstruct clusters dict: {id: {'center': v, 'members': [v_tuples]}}
            clusters = {}
            for cid, cdata in request.clusters.items():
                members = []
                for m in cdata.members:
    ...
    async def Query(self, request, context):
        try:
            ...
            results = await self._run(self.server.query, q_vecs, request.status)
            # results might be None if not responsible? Check server.py
            # If server returns None, return empty list
            if results is None:
                results = []
