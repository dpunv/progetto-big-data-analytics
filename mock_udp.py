...
class MockNode:
    def __init__(self, node_id, total_nodes):
        ...
        self.peers_ports = [BASE_UDP_PORT + i for i in range(1, total_nodes + 1) if i != node_id]
    ...
    def _calculate_simulated_load(self):
        ...
        if self.qps < 10: self.qps = 100 # Keep alive traffic
            ...
            try:
                ...
                if len(data) >= 8:
                    ...
                    if old_status != 'ONLINE':
                        log(self.node_id, f"Detected NODE-{sender_id} is back ONLINE! (Load: {sender_load:.2f})", Colors.OKGREEN)
        ...
        for pid, info in self.peer_table.items():
            if info['status'] == 'ONLINE':
                if (now - info['last_seen']) > TIMEOUT_LIMIT:
                    info['status'] = 'OFFLINE'
                    log(self.node_id, f"ALERT: NODE-{pid} detected OFFLINE (Timeout > {TIMEOUT_LIMIT}s)", Colors.WARNING)
...
if __name__ == "__main__":
    nodes = []
    print(f"{Colors.HEADER}=== AVVIO SIMULAZIONE UDP HEARTBEAT ({NODES_COUNT} NODI) ==={Colors.ENDC}")
