...
def query_vectors_generic(url, collection, query, topk):
    with GLOBAL_LOCK:
        ...
        grouped = [query_vectors(url, get_collection_name(collection, key), list(group), topk) for key, group in groupby(vectors_sorted, keyfunc)]
...
def insert_vectors_generic(url, collection, vectors, batch_size_retry, batch_size=256):
    with GLOBAL_LOCK:
        ...
        grouped = [insert_vectors(url, get_collection_name(collection, key), list(group), batch_size_retry, batch_size) for key, group in groupby(vectors_sorted, keyfunc)]
