...
class Server:
    ...
    def register_peers(self, peers_list):
        ...
        payload = {"id": get_id(), "peers": self.peers}
        try:
    ...
    def send_vectors(self, vectors_list: ListOfVectorsWithPayload):
        payload = {"id": get_id(), "content": vectors_list}
        try:
    ...
    def query_vectors(self, vectors_list: ListOfVectors):
        payload = {"id": get_id(), "query": vectors_list, "topk": 5}
        try:
    ...
    def metrics_snapshot(self):
        try:
            return requests.get(f"{self.url}/metrics_snapshot")
        except Exception as e:
            logger.error(f"Metrics snapshot failed on {self.id}: {e}")
            return None


def run_queries(servers: List[Server], query_vectors: ListOfVectors):
    logger.info(
        f"Distributing {len(query_vectors)} queries across {len(servers)} servers..."
    )
    results_by_query = {}
    for idx, query_vector in enumerate(query_vectors):
        ...
        response = target_server.query_vectors([query_vector])
...
def main():
    ...
    parser.add_argument(
        "--log-file", type=str, default="logs/client.log", help="Path to log file"
    )
    ...
    for server in servers:
        ...
        server.register_peers(
            [(s.id, s.url, s.grpc_url) for s in servers if s.id != server.id]
        )
    ...
    logger.info(
        f"Expected total vectors (with replicas): {expected_total_with_replicas}"
    )
    logger.info("=" * 60)
    ...
    logger.info(
        f"Starting parallel batch sending with {num_threads} threads for {total_batches} batches across {len(servers)} servers"
    )
    ...
    def send_batch_worker(args):
        ...
        logger.info(
            f"[{thread_name}] Sending batch {batch_num}/{total_batches} ({len(batch_data)} vectors) to {server.id}"
        )
        ...
        try:
            ...
            logger.info(
                f"[{thread_name}] Batch {batch_num} sent successfully to {server.id}"
            )
            ...
            if len(batch_data) > batch_size_send_retry:
                logger.warning(
                    f"[{thread_name}] Batch {batch_num} FAILED to {server.id} with size {len(batch_data)}. Retrying with batch_size={batch_size_send_retry}... Error: {e}"
                )

                fallback_batch_size = batch_size_send_retry
                total_sub_batches = (
                    len(batch_data) + fallback_batch_size - 1
                ) // fallback_batch_size

                try:
                    for i in range(total_sub_batches):
                        sub_batch = batch_data[
                            i * fallback_batch_size : (i + 1) * fallback_batch_size
                        ]
                        logger.info(
                            f"[{thread_name}] Sending sub-batch {i + 1}/{total_sub_batches} of batch {batch_num} to {server.id}"
                        )
                        server.send_vectors(sub_batch)

                    logger.info(
                        f"[{thread_name}] Batch {batch_num} sent successfully (via fallback) to {server.id}"
                    )
                    return (batch_num, True, None)

                except Exception as e2:
                    logger.error(
                        f"[{thread_name}] Batch {batch_num} FAILED during fallback to {server.id}: {e2}"
                    )
                    return (batch_num, False, str(e2))
            else:
                # If batch is already small, just fail
                logger.error(
                    f"[{thread_name}] Batch {batch_num} FAILED to {server.id}: {e}"
                )
    ...
    with ThreadPoolExecutor(
        max_workers=num_threads, thread_name_prefix="BatchSender"
    ) as executor:
    ...
    if int(config["num_before_clustering"]) <= vector_sent:
            ...
            if count_res:
                ...
                for node_id, count in counts.items():
                    if isinstance(count, int):
                        ...
                        logger.warning(
                            f"Invalid count received from {node_id}: {count}"
                        )
        ...
        if total < vector_sent * config["replicas"]:
            ...
            logger.warning(
                f"Missing vectors: {missing} ({missing / (vector_sent * config['replicas']) * 100:.2f}%)"
            )
        elif total > vector_sent * config["replicas"]:
            extra = total - (vector_sent * config["replicas"])
            logger.info(
                f"Extra vectors: {extra} ({extra / (vector_sent * config['replicas']) * 100:.2f}%)"
            )
        else:
    ...
    for idx, query_vector in enumerate(query_vectors):
        ...
        if system_results:
            sorted_results = sorted(
                system_results, key=lambda x: x["score"], reverse=True
            )[:5]
            logger.info("\nSystem Results (Top 5):")
            for i, result in enumerate(sorted_results, 1):
                logger.info(
                    f"  {i}. [{result['score']:.4f}] {result['payload']['string'][:75]}..."
                )
        distances_calcs = [
            (vector, utils.cosine_similarity(vector["embedding"], query_vector))
            for vector in data[:vector_sent]
        ]
    ...
    logger.info(f"\n{'=' * 80}")
    for server in servers:
        res = server.metrics_snapshot()
        res = res.json()
        logger.info(f"Metrics snapshot from {server.id}: {res}")
    logger.info("=" * 60)
