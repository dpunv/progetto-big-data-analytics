...
def heal_network(merge_indices=None):
    ...
    if merge_indices:
        ...
        print(
            "    Partial heal requested (not fully implemented helper, unblocking manually)"
        )
        pass
...
def print_server_status():
    ...
    for s in servers:
        ...
        print(
            f"      [{s.id}] {coord_status} vectors: {count:5d} (queue: {qsize:5d}) {hint_str}"
        )
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for i in range(phase1_batches):
        ...
        futures.append(
            executor.submit(
                send_batch, server_idx, batch_vectors, i + 1, phase1_batches
            )
        )
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for i in range(phase2_batches):
        ...
        batch_vectors = vectors[
            phase2_start + batch_size * i : phase2_start + batch_size * (i + 1)
        ]
        server_idx = i % len(servers)
        futures.append(
            executor.submit(
                send_batch, server_idx, batch_vectors, i + 1, phase2_batches
            )
        )
    concurrent.futures.wait(futures)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for i in range(phase4_batches):
        batch_vectors = vectors[
            phase4_start + batch_size * i : phase4_start + batch_size * (i + 1)
        ]
        ...
        futures.append(
            executor.submit(
                send_batch, server_idx, batch_vectors, i + 1, phase4_batches
            )
        )
    concurrent.futures.wait(futures)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for i in range(phase6_batches):
        batch_vectors = vectors[
            phase6_start + batch_size * i : phase6_start + batch_size * (i + 1)
        ]
        server_idx = i % len(servers)
        futures.append(
            executor.submit(
                send_batch, server_idx, batch_vectors, i + 1, phase6_batches
            )
        )
    concurrent.futures.wait(futures)
