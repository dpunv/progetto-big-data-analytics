...
def launch_and_wait_for_qdrant(qdrant_ports, timeout=60):
    ...
    while len(ready_nodes) < len(qdrant_ports):
        if time.time() - start_time > timeout:
            logger.error(
                f"Timeout! Only {len(ready_nodes)}/{len(qdrant_ports)} Qdrant nodes are ready."
            )
            return False
        ...
        for port in qdrant_ports:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"  [OK] Qdrant node on port {port} is ready. ({len(ready_nodes)}/{len(qdrant_ports)})"
                    )
...
def launch_servers(
    ...
    for i in range(1, len(fast_api_ports) + 1):
        ...
        logger.info(
            f"Started server '{node_id}' on port {fastapi_port}, Metrics={metrics_port}, (Qdrant: {qdrant_http_port}, PID: {proc.pid}). Log: {log_file}"
        )
...
def wait_for_servers(fast_api_ports, timeout=60):
    ...
    while len(ready_nodes) < len(fast_api_ports):
        if time.time() - start_time > timeout:
            logger.error(
                f"Timeout! Only {len(ready_nodes)}/{len(fast_api_ports)} servers are ready."
            )
            return False
        for i, port in enumerate(fast_api_ports):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"  [OK] Server on port {port} is ready ({len(ready_nodes)}/{len(fast_api_ports)})"
                    )
            except (
...
def collect_prometheus_metrics(start_port=10001, num_nodes=3):
    ...
    vector_routed = {
        k: v for k, v in counters.items() if k.startswith("vector_routed_")
    }
    ...
    if cluster_hits:
        ...
        print("-" * 70)
        for cluster, hits in sorted(
            cluster_hits.items(), key=lambda x: x[1], reverse=True
        )[:10]:
...
def main():
    ...
    config = {
        "servers": [
            {
                "id": f"node{i + 1}",
                "url": f"http://localhost:{FASTAPI_START_PORT + i + 1}",
                "grpc_url": f"localhost:{GRPC_START_PORT + i + 1}",
                "is_coordinator": False if i != 0 else True,
            }
            for i in range(N)
        ],
        "batch_size": 500,
    ...
    result = subprocess.run(
        [sys.executable, "client.py", "--log-file", "logs/client.log"]
    )
