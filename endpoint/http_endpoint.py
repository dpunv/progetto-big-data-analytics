...
class HTTPEndpoint(BaseEndpoint):
    ...
    async def start(self, ip: str, port: int):
        # Ensure max client size is large enough for vectors (100MB)
        ...
        self.app.router.add_post("/delete_vectors_local", self.handle_delete_vectors_local)
    ...
    async def handle_delete_vectors_local(self, request):
        try:
            data = await request.json()
            ids = data.get("ids")
            if ids:
                await self._run_sync(self.server.delete_vectors_local, ids)
