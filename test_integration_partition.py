...
def wait_for_queues(servers, timeout=None, check_hints=False):
    ...
    while time.time() - start < timeout:
            ...
            if total_hints > 0:
                for s in servers:
                    if s.hinted_handoff.count() > 0:
                        print(
                            f"    FAILURE: Server {s.id} has {s.hinted_handoff.count()} hints!"
                        )
                return False
        ...
        if time.time() - last_print > 5:
            ...
            total_hints = sum(s.hinted_handoff.count() for s in servers)
            print(
                f"    [Status] Queues: {total_q}, Clustering: {any_clustering}, Vectors stored: {total_stored}, Hints: {total_hints}, Dropped: {total_dropped}"
            )
...
def test_partition_tolerance_with_verification(cluster_setup):
    ...
    assert wait_for_network_convergence(
        servers, timeout=120
    ), "Network failed to converge at startup"
    ...
    assert (
        unique_count == batch1_size
    ), f"Expected {batch1_size} vectors, found {unique_count}"
    ...
    assert (
        unique_count_p2 == batch1_size + batch2_size
    ), "Lost vectors during partition write"
    ...
    assert (
        final_unique == total_vectors
    ), f"Expected {total_vectors} total vectors, found {final_unique}"
    ...
    assert (
        total_stored >= expected_replicated
    ), f"Replication looks low: {total_stored} < {expected_replicated}"
    ...
    for vec, payload in probes:
        ...
        if not found_target:
            ...
            assert (
                False
            ), f"Query mismatch using {inquirer.id}. Expected {payload}, got top results: {debug_info}"
