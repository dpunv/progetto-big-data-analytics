...
def launch_servers(
    ...
    for i in range(1, len(fast_api_ports) + 1):
        ...
        logger.info(
            f"Started server '{node_id}' on port {fastapi_port} (PID: {proc.pid})."
        )
...
def main():
    ...
    for server in servers_info:
        peers_list = [
            (s["id"], s["url"], s["grpc_url"])
            for s in servers_info
            if s["id"] != server["id"]
        ]
        payload = {"id": int(time.time()), "peers": peers_list}
        try:
    ...
    logger.info(
        f"Node URLs: {', '.join([f'http://localhost:{p}' for p in fast_api_ports])}"
    )
