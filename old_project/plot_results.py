...
def parse_report(filepath):
    ...
    for line in lines:
        ...
        if line.startswith("Throughput:"):
            current_scenario["throughput"] = float(
...
def plot_metrics(scenarios):
    ...
    plt.bar(
        [i - width / 2 for i in x], insert_lats, width, label="Insert", color="#2196F3"
    )
    plt.bar(
        [i + width / 2 for i in x], query_lats, width, label="Query", color="#FF9800"
    )
