        ...
        with results_lock:
            ...
            for server_id, data_server in local_per_server.items():
                if server_id not in results['per_server']:
                    results['per_server'][server_id] = {'insert': [], 'query': [], 'errors': 0}
...
def get_per_server_summary(per_server_data: Dict[int, Dict]) -> str:
    ...
    lines.append(f"  Fastest: Server {fastest[0]} ({fastest[1]['avg_latency']*1000:.2f} ms)")
    lines.append(f"  Slowest: Server {slowest[0]} ({slowest[1]['avg_latency']*1000:.2f} ms)")
    lines.append(f"  Speed Difference: {speed_diff:.1f}%")
    
    if speed_diff > 50:
        lines.append(f"  WARNING: Server {slowest[0]} is significantly slower!")
        lines.append(f"     Consider investigating load or resource issues.")
    elif speed_diff > 20:
...
def wait_for_clustered(servers: List[sv.Server], timeout: int = 120) -> bool:
    ...
    """
    logger.info(f"Waiting for all {len(servers)} servers to reach CLUSTERED status (timeout: {timeout}s)...")
...
def populate_and_wait_clustering(client: BenchmarkClient, data: List[Dict], servers: List[sv.Server], target_vectors: int = 10000):
    """
    ...
    if target_vectors < min_for_clustering:
        logger.info(f"Adjusting warmup count from {target_vectors} to {min_for_clustering} (clustering threshold)")
...
def run_suite(data_path: str, output_file: str, num_servers: int, replication_factor: int, warmup_count: int):
    ...
    # Create servers
    logger.info(f"Creating {num_servers} servers with replication factor {replication_factor}...")
    servers = []
    ...
    for name, config in SCENARIOS.items():
        ...
        for server_id in sorted(results['per_server'].keys()):
            ...
            report_section.append(f"  Server {server_id}: {srv_total} ops ({srv_insert_count} ins/{srv_query_count} qry), "
        ...
        if name != "sanity_check":
            for server_id, server_data in results['per_server'].items():
                if server_id not in all_per_server_data:
                    all_per_server_data[server_id] = {'insert': [], 'query': [], 'errors': 0}
    ...
    with open(output_file, "a") as f:
        ...
        for s in servers:
            ...
            f.write(f"  Server {s.get_id()}: {count} vectors, queue: {queue_size}, hints: {hints}, status: {status} {coord}\n")
