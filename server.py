...
def create_app(node: QdrantNodeWrapper) -> FastAPI:
    ...
    # Attach node instance to routers for dependency injection
    peer_router.node = node
    vector_router.node = node
    search_router.node = node
    admin_router.node = node
    
    # Include all routers (replaces ~500 lines of endpoint definitions!)
