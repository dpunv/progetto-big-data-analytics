...
class GRPCCommunicator(BaseCommunicator):
    """
    Communicator implementation using gRPC protocol.
    """

    async def send(self, ip: str, port: int, query: str, *args):
        """
        Sends data via gRPC. Matches the query to the appropriate gRPC method.
        """
        target = f"{ip}:{port}"
        options = [
                ...
                if query == "get_id":
                    ...
                    for cid, data in clusters_in.items():
                        ...
                        for m in data["members"]:
                            ...
                            members.append(
                                p2p_pb2.VectorComplete(
                                    values=mv,
                                    id=mid,
                                    payload=mp,
                                    cluster_id=mcid,
                                    timestamp=mts,
                                    version_node_id=mnid,
                                    destinations=mdests,
                                )
                            )

                        clusters_map[cid] = p2p_pb2.ClusterData(
                            center=center, members=members
                        )

                    assign_bytes = pickle.dumps(assignment_in)

                    req = p2p_pb2.SetClustersRequest(
                        clusters=clusters_map, assignment_pickle=assign_bytes
                    )
                    ...
                    for v_item in vecs_in:
                        proto_vecs.append(
                            p2p_pb2.VectorWithId(values=v_item[0], id=v_item[1])
                        )
                    ...
                    for item in resp.results:
                        results.append(
                            (list(item.vector), item.id, item.payload, item.similarity)
                        )
                    ...
                    for v in resp.vectors:
                        dests = (
                            frozenset(v.destinations) if v.destinations else frozenset()
                        )
                    ...
                    return {
                        "status": -1,
                        "error": f"Unknown gRPC method: {query}",
                        "response": None,
                    }
