...
class BenchmarkClient:
    ...
    def insert(self, vector: List[float], payload_text: str) -> Tuple[float, bool, str]:
        ...
        payload = {"id": req_id, "content": content}
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.error(
                    f"Insert failed with status {resp.status_code}: {resp.text}"
                )
    ...
    def query(self, vector: List[float]) -> Tuple[float, bool, str]:
        ...
        payload = {"id": req_id, "query": [vector], "topk": 5}
...
def get_per_server_summary(per_server_data: Dict[str, Dict]) -> str:
    ...
    if speed_diff > 50:
        ...
        lines.append("     Consider investigating network, load, or resource issues.")
    elif speed_diff > 20:
...
def wait_for_all_servers_clustered(urls: List[str], timeout: int = 120):
    ...
    """
    logger.info(
        f"Waiting for all {len(urls)} servers to reach CLUSTERED status (timeout: {timeout}s)..."
    )
...
def run_suite(urls_str, data_path, output_file):
    ...
    for name, config in SCENARIOS.items():
        ...
        for url in sorted(results["per_server"].keys()):
            ...
            report_section.append(
                f"  {url}: {srv_total} ops ({srv_insert_count} ins/{srv_query_count} qry), "
...
def load_urls_from_config(config_path: str = "config.json") -> str:
    ...
    try:
        ...
        if urls:
            ...
            urls_str = ",".join(urls)
            logger.info(
                f"Loaded {len(urls)} server URLs from {config_path}: {urls_str}"
            )
            return urls_str
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated server URLs (optional, reads from config.json if not specified)",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if urls is None:
        ...
        if urls is None:
            logger.error(
                "Could not load URLs from config. Please specify --urls or check config.json"
            )
            sys.exit(1)
