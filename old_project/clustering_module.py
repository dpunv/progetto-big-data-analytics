...
class MetaHNSW:
    ...
    def find_nearest_nodes(
        ...
        return sorted(
            [
                (cluster_id, dist)
                for cluster_id, dist in zip(cluster_ids[0], distances[0])
            ],
            key=lambda x: x[1],
        )

    def search_batch(
        self, query_vectors: np.ndarray, k: int = 1
...
def find_k_and_run_kmeans(X, max_k=30, random_state=42):
    ...
    if device.type == "cuda":
        ...
        logger.info("GPU ACCELERATION ENABLED: Apple MPS")
    else:
    ...
    if device.type != "cpu":
        try:
            ...
            logger.warning(
                f"Failed to move data to {device}: {e}. Falling back to CPU."
            )
    ...
    if n > SILHOUETTE_SUBSAMPLE_SIZE:
        ...
        logger.info(
            f"Using subsampling for silhouette score: {SILHOUETTE_SUBSAMPLE_SIZE}/{n} samples"
        )
    else:
    ...
    for k in k_range:
        if device.type == "cpu":
            ...
            kmeans = KMeans(
                n_clusters=k, max_iter=KMEANS_N_ITER, random_state=42, n_init=1
            )
            kmeans.fit(X)
            ...
            centroids_gpu, labels_gpu = kmeans_torch(
                X_tensor, num_clusters=k, niter=KMEANS_N_ITER, device=device
            )
    ...
    if best_centroids is not None:
        ...
        logger.warning(
            "Clustering failed. Using fallback: single cluster with mean centroid."
        )
...
def find_assignment(
    clusters: List[Tuple[str, int, List[float]]],
    all_nodes,
    replication_factor,
    beam_width,
) -> Dict[str, ListOfVectorsWithId]:
    """
...
def get_clusters(
    vectors: ListOfVectorsComplete, max_clusters=30
) -> Dict[VectorId, Tuple[Vector, ListOfVectorsComplete]]:
...
def get_node_assignment(
    clusters: Dict[VectorId, Tuple[Vector, List[VectorId]]], peers, replication_factor
) -> Dict[str, ListOfVectorsWithId]:
    logger.info(
        f"Computing node assignment for {len(clusters)} clusters across {len(peers)} peers"
    )
    ...
    assignment = find_assignment(
        request, [peer.id for peer in peers], replication_factor, 50
    )
...
def build_meta_hnsw(clusters: Dict[VectorId, Tuple[Vector, List[VectorId]]], dimension):
    logger.info(
        f"Building MetaHNSW index for {len(clusters)} clusters (dimension={dimension})"
    )

    clusters_adjusted = [
        (cluster_id, cluster_centroid)
        for cluster_id, (cluster_centroid, _) in clusters.items()
    ]
...
def find_batch(
    hnsw: MetaHNSW, vectors: Union[List[Vector], np.ndarray], k: int
) -> List[List[VectorId]]:
    """
