...
class PhiAccrualFailureDetector:
    ...
    def __init__(
        ...
        first_heartbeat_estimate_ms: float = 3000.0,
    ):
        """
        Args:
    ...
    def _update_statistics(self):
        ...
        if n > 1:
            squared_diff_sum = sum(
                (x - self._cached_mean) ** 2 for x in self.heartbeat_intervals
            )
...
class QdrantVectorStore:
    ...
    def _ensure_collection(self, vector_dim):
        if not self.collection_created:
            res = qdrant_module.create_collection(
                self.url, self.collection_name, vector_dim
            )
            if res:
    ...
    def has_vector(self, vec_id: int) -> bool:
        return (
            qdrant_module.retrieve_vector(self.url, self.collection_name, vec_id)
            is not None
        )
...
class Server:
    ...
    def similarity(self, vector):
        ...
        return max(
            [
                cosine_similarity(vector, cluster_center)
                for _, cluster_center in current_clusters
            ]
        )
    ...
    def route_vectors(self, vectors, top_k=3, use_all_peers=False):
        ...
        for vector in vectors:
            ...
            results[vec_id] = sorted(
                peer_similarities, key=lambda x: x[1], reverse=True
            )[:top_k]
        ...
        if self.cluster_index:
            ...
            if nearest_cluster_ids and self.cluster_to_destinations_cache:
                ...
                if best_cluster in self.cluster_to_destinations_cache:
                    return (
                        self.cluster_to_destinations_cache[best_cluster],
                        best_cluster,
                    )
    ...
    def _route_vectors_to_cluster(self, vectors: ListOfVectorsComplete, sender_status):
        ...
        if dropped_count > 0:
            print(
                f"DEBUG: Server {self.id} routing report: {dropped_count} DROPPED due to no destinations"
            )
            ...
            if status == "bootstrap":
                if self.i_am_coord():
                    ...
                    if coord:
                        ...
                        print(
                            f"DEBUG: Server {self.id} could not find coordinator to forward to!"
                        )
        ...
        with self.split_queue_lock:
            for vector in vectors:
                ...
                if cluster_id in self.splitting_clusters:
                    ...
                    print(
                        f"DEBUG: Server {self.id} buffered vector for splitting cluster {cluster_id}"
                    )
                else:
    ...
    def set_clusters(
        ...
        self.queue.put(
            (self._handle_set_clusters, (vectors_for_clusters, clusters, version))
        )

    def _handle_set_clusters(
        self, vectors_for_clusters, clusters: ListOfVectorsWithId, version=0
    ):
        with self.lock:
    ...
    def add_to_buffer(self, vectors: ListOfVectorsComplete):
        with self.lock:
            ...
            if self.status == "bootstrap":
                if (
                    len(self.vector_buffer) >= self.before_clustering
                    and not self.clustering_in_progress
                ):
    ...
    def clustering(self, vectors: ListOfVectorsComplete, min_k=MIN_K, max_k=MAX_K):
        ...
        if n_samples > SILHOUETTE_SUBSAMPLE_SIZE:
            ...
            print(
                f"DEBUG: Server {self.id} - Using subsampling for silhouette: {SILHOUETTE_SUBSAMPLE_SIZE}/{n_samples}"
            )
        else:
            sample_indices = None
            X_sample = X
        ...
        print(
            f"DEBUG: Server {self.id} - Evaluating k from {effective_min_k} to {effective_max_k}"
        )
        ...
        for k in k_range:
            try:
                ...
                print(
                    f"DEBUG: Server {self.id} - k={k:2d} | Silhouette Score: {score:+.4f}"
                )
                ...
                print(
                    f"DEBUG: Server {self.id} - Failed to compute clustering for k={k}: {e}"
                )
                continue
        ...
        return {
            i: {"center": best_centers[i], "members": members[i]} for i in range(best_k)
        }

    def _constrained_kmeans(self, X, k, max_size, max_iter=20):
        """
        K-Means with maximum cluster size constraint.
        Uses iterative reassignment to satisfy max_size.

        Args:
            X: Data matrix (n_samples, n_features)
            k: Number of clusters
            max_size: Maximum allowed points per cluster
            max_iter: Maximum iterations for reassignment logic
        ...
        for iteration in range(max_iter):
            ...
            if np.all(counts <= max_size):
                print(
                    f"DEBUG: Constrained KMeans converged in {iteration} iterations (sizes: {counts})"
                )
                break
            ...
            for cluster_idx in violation_indices:
                ...
                for i in range(moves_to_make):
                    _, pt_idx, target_c = move_proposals[i]
                    labels[pt_idx] = target_c
    ...
    def assign_clusters_to_peers(self, clusters, beam_width=50):
        ...
        for index, nodes_tuple in enumerate(best_assignment):
            for node in nodes_tuple:
                assignment[node].append(
                    (clusters_sorted[index][0], clusters_sorted[index][2])
                )
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for peer in peers_to_query:
                if peer_to_vec[peer.get_id()]:
                    futures.append(
                        executor.submit(
                            peer.search_vectors_local, peer_to_vec[peer.get_id()], top_k
                        )
                    )
    ...
    def get_load_stats(self) -> dict:
        ...
        for peer in self.get_reachable_peers():
            ...
            try:
                ...
                print(
                    f"DEBUG: Server {self.id} - Failed to get count from peer {peer.get_id()}: {e}"
                )
    ...
    def _calculate_split_parameters(
        self, cluster_id: int, n_subclusters: int = None
    ) -> dict:
        """
        Calculate K-means split for a cluster.
        Returns dict of {new_cluster_id: {"center": [...], "destinations": [peer_id, ...]}, ...}
        ...
        if n_vectors < n_subclusters:
            ...
            if len(candidates) >= self.replication_factor - 1:
                replicas = random.sample(candidates, self.replication_factor - 1)
        ...
        print(
            f"DEBUG: Server {self.id} - Constrained KMeans: n={n_vectors}, k={n_subclusters}, max_size={max_size}"
        )
        ...
        try:
            ...
            for i, vec in enumerate(cluster_vectors):
                vec_id = vec[1]
                labels_by_vector_id[vec_id] = int(labels[i])
            ...
            try:
                ...
                print(f"DEBUG: Server {self.id} - Standard K-means also failed: {e2}")
                new_id = self._generate_cluster_id()
    ...
    def _process_pending_vectors_after_split(self, old_cluster_id: int):
        ...
        if not pending:
            print(
                f"DEBUG: Server {self.id} - No pending vectors for cluster {old_cluster_id}"
            )
            return
    ...
    def _distribute_vectors_from_split(
        self, split_result: dict, is_coordinator: bool = True
    ) -> dict:
        """
            ...
            if peer_loads:
                ...
                if projected_load < underload_threshold:
                    ...
                    vectors_per_subcluster = (
                        total_vectors_in_split / n_subclusters
                        if n_subclusters > 0
                        else 0
                    )
                    ...
                    if projected_with_one >= underload_threshold * 0.8:
                        print(
                            f"DEBUG: Server {self.id} - Would be underloaded "
                ...
                if destination == self.id:
                    if not local_kept:
                        for vec in data["members"]:
                            self.store.insert(vec)
                        print(
                            f"DEBUG: Server {self.id} - Keeping (re-inserting) {len(data['members'])} vectors locally (cluster {new_cluster_id})"
                        )
                        local_kept = True
                else:
    ...
    def _get_underloaded_peers(
        self,
        count: int,
        sender_current_load: int = None,
        vectors_being_sent: int = None,
        n_subclusters: int = None,
        cluster_being_split: int = None,
        cluster_replicas: set = None,
    ) -> tuple:
        """
            ...
            try:
                ...
                if cluster_replicas and peer.get_id() in cluster_replicas:
                    try:
                        ...
                        print(
                            f"DEBUG: Server {self.id} - Peer {peer.get_id()} projected load: "
                        ...
                        print(
                            f"DEBUG: Server {self.id} - Peer {peer.get_id()} projected load (estimated): {load}"
                        )
    ...
    def _balance_cluster_sizes(
        self, subclusters: dict, centers: list, X: np.ndarray, target_size: int
    ) -> dict:
        """
    ...
    def trigger_rebalance(self) -> bool:
        ...
        if heaviest is None:
            print(
                f"DEBUG: Server {self.id} - No heavy cluster found on server {overloaded_id}"
            )
            return False
        ...
        print(
            f"DEBUG: Server {self.id} - Rebalancing complete, created {len(new_clusters)} subclusters"
        )
        return True

    def _broadcast_cluster_update(
        self, new_clusters: dict, old_cluster_id: int, split_plan: dict = None
    ):
        """
        ...
        for peer in self.get_reachable_peers():
            if peer.get_id() != self.id:
                try:
                    ...
                    print(
                        f"DEBUG: Server {self.id} - Failed to update peer {peer.get_id()} clusters: {e}"
                    )

    def _calculate_destinations_for_cluster(
        self, cluster_id: int, force_include: set = None, projected_usage: dict = None
    ) -> set:
        """
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Internal port for server-to-server communication",
    )
    parser.add_argument(
        "--inter-port",
        type=int,
        default=None,
        help="External port for client communication (optional)",
    )
    parser.add_argument("--qdrant", type=str, default=None, help="Qdrant URL")
    parser.add_argument(
        "--endpoint", type=str, default="QUIC", help="Endpoint type (QUIC, HTTP, GRPC)"
    )
    parser.add_argument(
        "--coordinator", action="store_true", help="Is this node the coordinator?"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--replication-factor", type=int, default=3, help="Replication factor"
    )
    ...
    print(
        f"Server {args.id} started. Intra: {args.intra_port}, Inter: {args.inter_port}, Coord: {args.coordinator}"
    )
                ...
                for p in peers_data:
                    if isinstance(p, dict):
                        ...
                        if p_ip and p_port:
                            server.add_peer(p_ip, int(p_port))
                            print(
                                f"[ID {args.id}] Added peer {p_id} ({p_ip}:{p_port}) from file"
                            )
        except Exception as e:
