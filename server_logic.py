...
class ServerApp:
    ...
    def query(self, query: ListOfVectors, topk: int, request_id: int):
        with metrics.REQUEST_LATENCY.labels(operation='global_query').time():
            logger.info(f"[Global Query] Processing query (ReqID: {request_id}).")
            
            if self.meta_hnsw is None or self.status == ServerStatus.CLUSTERING:
                if self.i_am_coord():
                    return self.linear_search(query, topk)
                else:
                    coord = self.coordinator()
            ...
            def query_peer_task(peer, queries):
                return (peer.id, True, peer.query_peer(queries, topk, request_id))
